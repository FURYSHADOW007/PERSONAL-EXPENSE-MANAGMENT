...
if monthly_expense > limit:
    st.error(f"⚠️ Over Budget! ₹{monthly_expense:.2f} spent of ₹{limit}")
else:
    st.success(f"✅ Within Budget: ₹{monthly_expense:.2f} of ₹{limit}")

# --- Reset Button ---
with st.expander("⚠️ Danger Zone: Reset All Transactions"):
...
if not df.empty:
    ...
    index_to_edit = st.number_input("Enter row number to edit/delete (starts at 0)", min_value=0, max_value=len(df_display)-1)
    row = df_display.iloc[index_to_edit]
    ...
    with st.form("edit_form"):
        ...
        delete = col2.form_submit_button("🗑️ Delete Transaction")
